...
```bash
npm test                    # typecheck + particles + shaders + UI parity + UI behavior + editor smoke
...
> **Nothing runs these automatically.** That is currently this repo's weakest link, and not hypothetically: the geometry-parity suite — the mechanical enforcement the locked scaling system depends on — sat **red** through several commits, because having a test and running it are different things. Two of the three UI bugs it was meant to catch reached `src/game/ui/mainLayout.json`. Until `npm test` runs on push, every guarantee in this document is only as good as someone remembering to type it.

Six suites, each covering something that can't be caught by reading the code:
...
- **`tests/render-defaults-parity.test.mjs`** guards the *other* half of that same contract: every `?? fallback` on a schema field. It fails the build if either render path defaults a field with `||` (which swallows an authored `0`/`""` and is precisely how `fontSizePct: 0` came to preview at 4% and ship at 0px) or if the two files disagree on a literal, and it asserts both sides still rank `z-index` through `buildStackRanks` rather than emitting an invalid fractional value. Its small exemption list — the video playback flags the editor never reads because it deliberately doesn't play video — is itself checked for staleness, so it can't rot into a hiding place for real divergence.
- **`tests/ui-layout.test.mjs`** exercises the real `UILayout` against a real (jsdom) DOM: every element type builds the nodes it claims to, documented defaults are the ones that actually apply, `updateScale` re-derives px geometry/font-size/scaled styles and ignores a zero-size container, and the interactive surface (values, states, actions, `dispose`) behaves as the editor's UI promises.
- **`tests/ui-editor.test.mjs`** drives the editor page through its real controls rather than its internals — which are closure-private inside the page's IIFE anyway, and, more to the point, "the button does the thing" is what actually regresses. Covers inserting every type, the transform rig, sectioned and multi-select property editing, clipboard/undo, validation, view controls, and a saved-in-2024-style layout still loading unchanged. Its `beforeEach` asserts the page threw nothing during boot, which is what catches the temporal-dead-zone class of error (a `const` referenced above its declaration) that a syntax check can't.

`npm test` also runs **`scripts/test-ui-editor.js`**, an older end-to-end smoke pass over the editor (insert → edit → drag → upload an image → Save) that asserts the emitted JSON matches the schema `UILayout.ts` expects. It predates the suites above and overlaps them, but it exercises the whole flow in one sequence rather than a feature at a time, which is a different kind of check — it is what would catch two individually-correct steps that break when run back to back.
...
The two UI suites stub `getBoundingClientRect` (jsdom has no layout engine). The editor one makes that stub honor the canvas's own `scale()` transform on purpose: the editor divides the measured box by `zoom` to recover logical pixels, so a stub returning a constant would make zooming *appear* to shrink the design resolution — a bug the real browser doesn't have, and one that would send whoever reads the failure off chasing the wrong thing. Chrome-placement tests stub each panel at a realistic size for the same reason: `chromeInsets()` measures, so a uniform stub would make every inset identical and prove nothing.

### Known gaps, in the order worth closing them

1. **No CI.** See the note above. A workflow running `npm test` on push is the single highest-leverage thing missing from this repo, and it is roughly fifteen minutes of work.
2. **No collision tests.** `collision/ColliderManager.ts` and `collision/intersect.ts` have zero coverage, while being among the most-churned files in recent history (two of the last three commits are collision fixes). The narrow-phase maths is pure and DOM-free — the same shape as the particle suite, and just as cheap to write.
3. **The build's own compatibility scan isn't a gate.** `build.sh` already writes `compatibilityWarnings` into `dist/build-report.json`, catching the exact syntax classes that fail ad-network review — but nothing fails the build when that array is non-empty. It is a report nobody is required to read.
4. **No visual regression.** jsdom applies no styles and lays nothing out, so no test here can catch a wrong gradient, a rotated element landing a few pixels off, or text clipping at an extreme aspect ratio. That last one has shipped before (see the stage-transform history in the `ui/UILayout.ts` section), and the locked-system rules say to verify by measuring a real browser — which means Playwright, not jsdom.
5. **No boot-sequence test.** The suites cover pieces; ordering bugs (`bindIon` before `Game.create()`, dev hooks installed before `__onGameReady` fires) live in the sequence between them.

**What these do not cover:** pixel appearance. jsdom applies no styles and lays nothing out, so these verify structure, computed inline values, and behavior — not that a gradient looks right or that a rotated element visually lands where you expect. Confirm those in a browser, and per the locked-system section, verify any resize change at an aspect ratio far from the design's own by measuring real `getBoundingClientRect()` values.
